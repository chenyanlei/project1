...
<script>
    ...
    var m_ptype="<?php echo $p_type;?>";
    var m_id="<?php echo $id;?>";
...
//        var travel_detail = $("#travel_desc").val() ;
        ...
        add_one_day_data(obj, m_arr_days_travel.length + 1) ;
    ...
    function add_one_day_data(obj, days_num) {
        console.log(obj) ;
        ...
        span_edit.innerHTML=  "编辑" ;
        span_edit.className="com_travel_info_edit";
        span_edit.setAttribute("data", days_num) ;
        span_edit.setAttribute("style", "float:left") ;
        div_title.appendChild(title) ;
        div_title.appendChild(span_edit) ;
        var div_clear=document.createElement('div');
        div_clear.setAttribute("style","clear:both") ;
        div_title.appendChild(div_clear) ;
        ...
        detail.className="div_intro"

        div.appendChild(detail) ;
        $("#travel_detail").append(div) ;
    }

    var m_cur_edit_index ;

    $(function(){

        $("#travel_detail").delegate("span","click", function() {
            var data = $(this).attr("data") ;
            m_cur_edit_index = data -1 ;
            var day_obj = m_arr_days_travel[m_cur_edit_index] ;
            index = layer.open({
                type: 2,
                title:"行程编辑",
//                btn: ['确定'],
                shade: [0.8, '#393D49'],
                area: ['1000px', '500px'],
//                content: $("#travel_detail").children().eq(0),
//                content:$("#travel_detail_add"),
                content: "/product_edit/get_day_edit_page?cur_edit_index=" +m_cur_edit_index + "&id=" + day_obj.id + "&pid=" + m_pid + "&p_type="+m_ptype  ,
        ...
        if(isset($travel_intro)) {
             foreach($travel_intro as $travel_intro_item) { ?>
                var obj = new Object() ;
                obj.id = "<?php echo $travel_intro_item['id'];?>" ;
                ...
                obj.travel_detail =  <?php echo json_encode($travel_intro_item['travel_intro']);?> ;
                add_one_day_data(obj, date_num) ;
                date_num ++ ;
                m_arr_days_travel.push(obj) ;
