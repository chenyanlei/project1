...
Jumei.MobileBind = {
    ...
    from: $('#JS_form_data').val(), //手机绑定来源 from参数
    operationtype_balance: 'operationtype_user_balance', //余额手机验证支付
    operationtype_old_bind: 'operationtype_user_old_bind_mobile',//切换绑定 获取验证吗
    operationType_bind: 'operationtype_user_bind_mobile', //手机绑定
    is_cart: 0,
    init: function (){
        Jumei.MobileBind._init_phone_data();
    ...
    //初始化手机绑定来源 参数
    _init_phone_data: function () {
        var $checkbox = $('#use_balance_checkbox'),
            is_presale = $checkbox.attr('is_presale');

        if (is_presale) {
            Jumei.MobileBind.operationType_bind = 'operationtype_user_bind_mobile';
            Jumei.MobileBind.operationtype_balance = 'operationtype_user_balance';
        }
    },
    ...
    set_cart_check:function(){
        $("#mobile_confirm").hide();
        $("#cboxOverlay").hide();
        $("#use_balance_checkbox").attr("is_mobile_checked","true");
        $("#use_gift_card_checkbox").attr("is_mobile_checked","true");
        $("input[thisClicked='true']").click().change();
    ...
    reBindMobileCheck: function (){
        ...
        if ($('.mobile_subscribe #rebindcheck').attr('is_rebind') || $('.mobile_subscribe #bind').is(':visible')) {//如果是解绑 否则 余额验证码直接支付
            ...
            action = 'rebind_check';
            if ($('.mobile_subscribe #rebindcheck').is(':visible')) {
                operationType = Jumei.MobileBind.operationtype_old_bind;
            }
        }
