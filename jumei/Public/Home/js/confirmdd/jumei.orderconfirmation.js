...
*/
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data){
                        for(var x in data){
                            for(var y in data[x]){
                                html += div.replace('{discount_icon}',  data[x][y].type === 'promocard' ? '满返' : '满返红包').replace('{discount_main}', data[x][y].sale_name).replace("{promocardReplace}",
                                    data[x][y].type === 'promocard' ?  '' : '分为50元红包发放；');
                            }
                        }
    ...
    enableOptionManager: function() {
        ...
        $("#address_wrap").delegate("div[selector='new_address'] input, div[selector='old_address'] input","click",function(){
            if ($(this).closest('.option_box').attr('invalid_structure_code') == 'false') {
                Jumei.OrderConfirmation.on_address_changed();
            }
        });
    ...
    enableBalanceConfirm:function(){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                // close
                var cardInput_box = $("#use_gift_card_checkbox");
                close.click(function(e){
                    e.preventDefault();
                    mobile_confirm.hide();
                    $("#cboxOverlay").hide();
                    input_box.attr("checked", false);
                    cardInput_box.attr("checked", false);
                });
                // show pop box

                //给聚美余额的checkBox绑定点击事件；

                $('#paytype_gateway').delegate('input:checkbox','click',useBalanceTelphoneCheck);

/*               input_box.click(function(e){
                    var self = $(this),
                        checked = self.attr("checked"),
                        is_mobile_checked = self.attr("is_mobile_checked");
                    $('input[thisClicked]').attr('thisClicked', false);
                    self.attr('thisClicked', 'true');
                    if ( checked && is_mobile_checked != "true" ){
                        ...
                        mobile_confirm.find("#change_code").trigger("click");
                    }
                });*/
        ...
        var useBalanceTelphoneCheck = function(){
            //余额支付安全验证
            var self = $(this),
                checked = self.attr("checked"),
                is_mobile_checked = self.attr("is_mobile_checked");
            ...
            if ( checked && is_mobile_checked != "true" ){
                var top = $(window).scrollTop() + 200 ,
                    left = $(window).scrollLeft() + parseInt( ($("body").width() - mobile_confirm.width() ) / 2 );
                mobile_confirm.css({"left":left,"top":top}).show();
                $("#cboxOverlay").css("opacity", "0.2").show().unbind("click");
...
*/
    ...
    updateTotalSum : function(value){
        $(".cart_products #cart_total").html(value);
        $("#cart_total2").html(value);
        if(window.gift && window.gift.updateGiftCardDiv) {
            window.gift.updateGiftCardDiv();
        }
    }
    ,
                ...
                $.post(
                    ...
                    function(data){
                        if(data.status == "success"){
                            ...
                            Jumei.OrderConfirmation.updateTotalSum(data.total_amount);
    ...
    change_balance_text:function(need_amount){
        ...
        if (need_amount >0 ) {
			var proceed_notice = '<label for="use_balance_checkbox">使用账户余额付款<span class=""><font class="pink">￥</font><span id="account_balance" class="pink">'+balance
            ...
            var proceed_notice = '<label for="use_balance_checkbox"><span id="paytype_balance" style="">使用账户余额付款 </span><span class=""><font class="pink">￥</font><span class="pink">'+cart_total
