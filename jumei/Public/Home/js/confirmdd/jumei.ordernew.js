        ...
        $('.gateway_ul>li').click(function(e){
            ...
            if ($input.attr('disabled') === false && !$this.hasClass('weihu')) {
                ...
                if ($input.val() === 'COD') {
                    ...
                    /*var clickedTime = $input.data('time') || 0;
                    var currentTimeStamp = e.timeStamp;
                    var gap = currentTimeStamp - clickedTime;
                    $input.data('time',currentTimeStamp);
                    var param = gift.getParameter();
                    var checked = param.cbx1.is(':checked') || param.cbx2.is(':checked');*/
                    ...
                    /*if(gap > 100 && checked)//多重点击...*/
                    cod_user_balance.html("亲，货到付款不支持余额和礼品卡支付哦");//货到付款文案显示
                    /*if(gap > 100 && !checked)
                        cod_user_balance.html("");//货到付款文案显示*/
                } else {
                    gift.enableAll();
